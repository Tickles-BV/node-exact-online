...
/**
 * Dirty hack to enable authorization if we already have the refresh and accesstoken.
 *
 * @param accesstoken
 * @param refreshtoken
 * @param callback
...
Client.prototype.setToken = function(accesstoken, refreshtoken, callback) {
    this.oauth.authorized = true;
    this.oauth.token = accesstoken;
    this.oauth.refreshToken = refreshtoken;
    // todo pass this the expiresin value
    this.oauth.expires = Date.now() + (parseInt(1000) * 1000);
}
  ...
  if(requiresAuth) {
    ...
    self.checkAuth(function(err, isValid, token) {
      ...
      if(isValid) {
        ...
        headers['Authorization'] = 'Bearer ' + self.oauth.token;
